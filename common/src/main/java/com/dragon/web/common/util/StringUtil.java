...
public class StringUtil {
    ...
    public static boolean exists(String value, String[] source, boolean caseInsensitive, boolean useRegex) {
        if (source != null) {
            int len = source.length;

            for(int i = 0; i < len; ++i) {
                if (useRegex) {
                    Pattern p = caseInsensitive ? Pattern.compile(source[i], 2) : Pattern.compile(source[i]);
                    if (p.matcher(value).find()) {
                        return true;
                    }
                } else if (source[i].equals(value) || source[i].equalsIgnoreCase(value) && caseInsensitive) {
                    return true;
                }
            }
        }
    ...
    public static boolean isFalse(String value) {
        return "0".equals(value) || "false".equalsIgnoreCase(value) || "否".equals(value);
    }
    ...
    public static double parseDouble(String value, double defaultDouble) {
        ...
        try {
            val = Double.parseDouble(value.replaceAll(",", "").replace("+", ""));
        } catch (Exception var6) {
            ;
        }
    ...
    public static Enum<?> parseEnmu(Class<?> em, String value, Enum<?> defaultVal) {
        Enum en = null;

        try {
            en = (Enum)em.getMethod("valueOf", String.class).invoke(em, value);
    ...
    public static String limitBytes(String str, int maxBytes) {
        if (isEmpty(str)) {
            ...
            return byteLength > maxBytes ? new String(b, 0, maxBytes) : str;
        }
    }
    ...
    public static String[] split2Array(String string, String regex) {
        return isEmpty(string) ? new String[0] : string.split(regex);
    }
